...
int main()
{
	...
	// Инициализация на играта (раздаване, тесте и т.н.) — ще го допишеш
	...
	while (true) {
		...
		std::cout << "Player " << currentPlayer + 1 << " — Your cards:\n";
		for (int i = 0; i < p.cardCount; i++) {
			std::cout << "[" << i << "] ";
			printCard(p.hand[i]);
			std::cout << " ";
		...
		bool hasMove = false;
		for (int i = 0; i < p.cardCount; i++)
			if (isValidMove(p.hand[i])) hasMove = true;

		if (!hasMove) {
			std::cout << "No suitable cards. Drawing...\n";
			drawCard(p);
			nextPlayer();
			continue;
		}
		...
		if (choice < 0 || choice >= p.cardCount || !isValidMove(p.hand[choice])) {
			std::cout << "Invalid move. Try again.\n";
			continue;
		}

		topCard = p.hand[choice];
		activeColor = topCard.color;

		// Премахване на картата от ръката
		for (int i = choice; i < p.cardCount - 1; i++)
			p.hand[i] = p.hand[i + 1];
		p.cardCount--;
